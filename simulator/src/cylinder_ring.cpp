...
bool CylinderRing::intersect(const Ray &r, Hit &h) const {
  ...
  if (outer && (outer_t < h.getT())) {
    h.set(outer_t,this->getMaterial(),outer_normal, (Shape*)this);
    answer = true;
  }
  if (inner && (inner_t < h.getT())) {
    h.set(inner_t,this->getMaterial(),-inner_normal, (Shape*)this);
    answer = true;
  }
  if (top && (top_t < h.getT())) {
    h.set(top_t,this->getMaterial(),top_normal, (Shape*)this);
    answer = true;
  }
  if (bottom && (bottom_t < h.getT())) {
    h.set(bottom_t,this->getMaterial(),-bottom_normal, (Shape*)this);
    answer = true;
  }
  return answer;
} 

BoundingBox CylinderRing::getBoundingBox() const {
    Vec3f extents = Vec3f(outer_radius, height/2.0, outer_radius);
...
void CylinderRing::addRasterizedFaces(Mesh *m, ArgParser* /* args */) {
  int crr = CylinderRing::rasterization_count;
  int i;
  int va,vb,vc,vd;
  Vertex *a,*b,*c,*d;
