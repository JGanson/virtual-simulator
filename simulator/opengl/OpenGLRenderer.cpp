...
// Move the fluid simulation forward one step.
void Step();
...
// ==================================================================
/// Animates whatever needs animating.
void Animate() {
    bool do_repack = false;
...
OpenGLRenderer::OpenGLRenderer(MeshData *_mesh_data, ArgParser *args) {
    ...
    */
        Matrix ProjectionMatrix = GLOBAL_args->camera->getProjectionMatrix();
        Matrix ViewMatrix = GLOBAL_args->camera->getViewMatrix();
...
void OpenGLRenderer::drawMesh() const {
    ...
    glDisable(GL_CULL_FACE); // don't cull, i.e. for the wine glass scene
