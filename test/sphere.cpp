...
void Sphere::addRasterizedFaces(Mesh *m, ArgParser * /*args*/) {
    ...
    int va,vb,vc,vd;
    Vertex *a,*b,*c,*d;
    int offset = m->numVertices(false); //vertices.size();

    // place vertices
    m->addVertex(center+radius*Vec3f(0,-1,0), 0);  // bottom
    for (j = 1; j < v; j++) {  // middle
        for (i = 0; i < h; i++) {
            float s = i / float(h);
            float t = j / float(v);
            m->addVertex(ComputeSpherePoint(s,t,center,radius), 0);
        }
    }
    m->addVertex(center+radius*Vec3f(0,1,0), 0);  // top
