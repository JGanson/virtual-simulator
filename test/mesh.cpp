...
Vertex* Mesh::addVertex(const Vec3f &position,int index) {
    int in = numVertices(false);
    index += in;
...
Face* Mesh::addFace(Vertex *a, Vertex *b, Vertex *c, Vertex *d, Material *material, enum FACE_TYPE face_type) {
    ...
    } else if (face_type == FACE_TYPE_PERSON) {
        person_quads.push_back(f);
...
void Mesh::Load(std::ifstream& file, Material* mat, const Matrix& transform) {
    ...
    while (std::getline(file, line_buf)) {
        ...
        if (token == "material") {
            ...
            if (person) {
                addPersonVertex(pos,base);
            }else {
                addVertex(pos,base);
            }
        } else if (token == "vt") {
            assert (numVertices(person) >= 1);
            float s,t;
            ...
            getVertex(numVertices(person)-1)->setTextureCoordinates(s,t);
        } else if (token == "vn") {
            ...
            if (vert_a < base ) {
                ...
                std::cerr << "   note: in line " << line_num << ", '" << line_buf << "'\n";
                std::cerr << "   note: the base is " << base << " and there are " << numVertices(person) << " so far.\n";
                bad = true;
            }
            ...
            if (i == 3) { 
                if (person) {
                    Vec3f pos = 0.5 * getPersonVertex(vert_a - numVertices(false))->get() + 0.5 * getPersonVertex(vert_c - numVertices(false))->get();
                    addPersonVertex(pos,base);
                }else {
                    Vec3f pos = 0.5 * getVertex(vert_a - numVertices(true))->get() + 0.5 * getVertex(vert_c - numVertices(true))->get();
                    addVertex(pos,base);
                }
                vert_d = numVertices(person)+numVertices(!person)-1;
            } else {
            ...
            if (person) {
                addpersonQuad(getPersonVertex(vert_a - numVertices(false)),
                    getPersonVertex(vert_b - numVertices(false)),
                    getPersonVertex(vert_c - numVertices(false)),
                    getPersonVertex(vert_d - numVertices(false)),
                    active_material);

            } else {
                addOriginalQuad(getVertex(vert_a - numVertices(true)),
                    getVertex(vert_b - numVertices(true)),
                    getVertex(vert_c - numVertices(true)),
                    getVertex(vert_d - numVertices(true)),
                    active_material);
            }
...
Vertex* Mesh::AddEdgeVertex(Vertex *a, Vertex *b) {
    ...
    v = addVertex(pos,numVertices(false));
...
Vertex* Mesh::AddMidVertex(Vertex *a, Vertex *b, Vertex *c, Vertex *d) {
    ...
    Vertex *v = addVertex(pos, numVertices(false));
    v->setTextureCoordinates(s,t);
    return v;
}
